pipeline {
    ...
    stages {
     ...
     stage ("Docker Build") {
           ...
           steps{
             // docker login  
             bat(script: 'docker build -t antony0618/servicenet6v1 .' , returnStdout:true);
             bat(script: 'docker push antony0618/servicenet6v1' , returnStdout:true);  
           }
       }   
        ...
        stage ("Deploy AKS") {
           steps {
            bat(script: 'az aks get-credentials --resource-group Devops --name aksDevopssetiembre  & kubectl config get-contexts --kubeconfig=%KUBE_PATH_CONFIG%', returnStdout: true);
            bat(script: 'kubectl config use-context aksDevopssetiembre  --kubeconfig=%KUBE_PATH_CONFIG%', returnStdout: true);
